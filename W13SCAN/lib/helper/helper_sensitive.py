...
def sensitive_page_error_message_check(source) -> list:
    errors = [
        ...
        {'regex': '\(generated by waitress\)', 'type': 'Waitress Python server'}, {
            ...
            'type': 'WebSEAL'},
        {'regex': 'ASPNETPathDisclosure',
         'type': "<title>Invalid\sfile\sname\sfor\smonitoring:\s'([^']*)'\.\sFile\snames\sfor\smonitoring\smust\shave\sabsolute\spaths\,\sand\sno\swildcards\.<\/title>"},
        {'regex': 'Struts2DevMod',
         'type': 'You are seeing this page because development mode is enabled.  Development mode, or devMode, enables extra'},
        {'regex': 'Django DEBUG MODEL',
         'type': "You're seeing this error because you have <code>DEBUG = True<\/code> in"},
        {'regex': 'RailsDevMode', 'type': '<title>Action Controller: Exception caught<\/title>'},
        {'regex': 'RequiredParameter', 'type': "Required\s\w+\sparameter\s'([^']+?)'\sis\snot\spresent"},
        {'regex': 'Thinkphp3 Debug', 'type': '<p class="face">:\(</p>'}
    ]
    r = []
    for error in errors:
        regex = error["regex"]
